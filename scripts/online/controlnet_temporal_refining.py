...
from models.augmentation.controlnet_temporal_refining import ControlnetTemporalRefining
...
augmentation_model = ControlnetTemporalRefining(prompt="", guidance=1.0)
guidance = 10.0
...
for prompt in ALL_PROMPTS:
    ...
    run_name = f"online/controlnet_temporal_refining/{re.sub('[^0-9a-zA-Z]+', '-', prompt)}"
    if RESULT_DIR.joinpath(run_name).joinpath("after", "log.csv").exists():
        continue
    augmentation_model.prev_image = None
