...
from augment.domain import WEATHERS, COUNTRIES, LOCATIONS, CITIES
...
sd_inpainting = SDInpainting("", guidance=2.0)
# sd_inpainting.optimize()
...
def get_sd_inpainting_callbacks(simulator, run_name, prompt, guidance):
    ...
    sd_inpainting.prompt = prompt
    sd_inpainting.guidance = guidance
...
for domain in CITIES:
    prompt = f"a street in {domain}, photo taken from a car"
    for guidance in [2.5, 5.0, 7.5, 10, 12.5]:
        run_name = f"sd_inpaiting/{prompt.replace(' ', '_')}-{str(guidance).replace('.', '_')}"
