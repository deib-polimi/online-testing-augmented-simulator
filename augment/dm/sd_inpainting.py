...
class SDInpainting:
    ...
    def optimize(self):
        ...
        # self.pipe.vae.decode = torch.compile(self.pipe.vae.decode)
    ...
    def forward(self, x):
        ...
        # Image.fromarray(self.segmentation_mask_model(x)[1]).save("mask.jpg")
        # Road is assigned class '0', remove everything else
        mask_image = torch.tensor((mask_image != 0).astype(np.uint8)).to(DEFAULT_DEVICE)
        augmented_image = self.pipe(prompt=self.prompt, image=x, mask_image=mask_image, height=h * 2,
...
if __name__ == '__main__':
    ...
    # image = Image.open("/media/banana/data/dataset/deepnurse/mountain-sunny-day-mpc-60/IMG/center_2022_05_18_11_52_07_788.jpg")
    # image = Image.open("shift.jpg")
    # image = image.resize((320,200))
    # image = image.crop((0, 40, 320, 200))

    model = SDInpainting("a street in the usa, photo taken from a car, photorealistic, high quality", 7.5)
