...
from diffusers import EulerAncestralDiscreteScheduler, \
    AutoPipelineForInpainting, StableDiffusionImg2ImgPipeline
...
class SDInpaintingSD:
    ...
    def __init__(self, prompt: str, guidance: float, num_inference_step: int = 30, strength=0.3):
        ...
        self.pipe_sd = StableDiffusionImg2ImgPipeline.from_pretrained("runwayml/stable-diffusion-v1-5",
                                                                      torch_dtype=torch.float16, variant="fp16")
    ...
    def forward(self, x):
        ...
        image = self.pipe_sd(prompt=self.prompt, image=image, strength=self.strength, height=h * 2,
                             width=w * 2, num_inference_steps=self.num_inference_step,
...
if __name__ == '__main__':
    ...
    model = SDInpaintingSD("a street in the usa with an ambulance, photo taken from a car", guidance=7.5,
                           num_inference_step=30, strength=0.5)
    n_runs = 6
    inference_times = []
    for i in range(n_runs):
        ...
        torchvision.utils.save_image(result, f"sd_inpainting_img2img_{i}.jpg")
