...
class ControlnetTemporalRefining:

    def __init__(self, prompt: str, guidance: float, num_inference_step: int = 50, strength: float = 0.5,
    ...
    def forward(self, image):
        ...
        if self.prev_image == None:
            ...
            self.prev_image = inpainted_image
        ...
        control_image = self.hint_generator(
            ...
            image_resolution=h,
            detect_resolution=h)
        refined_image = self.refining_pipe(prompt=self.prompt, image=self.prev_image, control_image=control_image,
                                           strength=self.strength, height=h * 2, width=w * 2,
                                           num_inference_steps=self.num_inference_step, guidance_scale=self.guidance,
                                           output_type='pt').images

        self.prev_image = refined_image
        # 6. Resize to original image and return
...
if __name__ == '__main__':
    ...
    n_runs = 10
    base_folder = RESULT_DIR.joinpath("investigation", "offline", "controlnet_temporal_refining")
    ...
    model = ControlnetTemporalRefining(prompt="", guidance=10)
